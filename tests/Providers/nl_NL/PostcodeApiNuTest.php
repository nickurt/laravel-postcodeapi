...
class PostcodeApiNuTest extends TestCase
{
    ...
    public function setUp(): void
    {
        $this->postcodeApiNu = (new PostcodeApiNu)
            ->setRequestUrl('https://postcode-api.apiwise.nl/v2/addresses/?postcode=%s&number=%s')
    ...
    {
        Http::fake(['https://postcode-api.apiwise.nlx/v2/addresses/?postcode=1118CP&number=' => Http::response('{"_embedded":{"addresses":[]},"_links":{"self":{"href":"https://postcode-api.apiwise.nl/v2/addresses/?postcode=1234AB"}}}')]);

        $address = $this->postcodeApiNu->find('1234AB');
    ...
    {
        Http::fake(['https://postcode-api.apiwise.nl/v2/addresses/?postcode=1118CP&number=202' => Http::response('{"_embedded":{"addresses":[{"purpose":"kantoorfunctie","postcode":"1118CP","surface":16800,"municipality":{"id":"0394","label":"Haarlemmermeer"},"city":{"id":"1618","label":"Schiphol"},"letter":null,"geo":{"center":{"rd":{"type":"Point","coordinates":[111396.536,479739.602],"crs":{"type":"name","properties":{"name":"urn:ogc:def:crs:EPSG::28992"}}},"wgs84":{"type":"Point","coordinates":[4.7479077,52.3038972],"crs":{"type":"name","properties":{"name":"urn:ogc:def:crs:OGC:1.3:CRS84"}}}}},"nen5825":{"postcode":"1118 CP","street":"EVERT VAN DE BEEKSTRAAT"},"addition":null,"number":202,"year":2007,"province":{"id":"27","label":"Noord-Holland"},"id":"0394200001001951","type":"Verblijfsobject","street":"Evert van de Beekstraat","_links":{"self":{"href":"https://postcode-api.apiwise.nl/v2/addresses/0394200001001951/"}}}]},"_links":{"self":{"href":"https://postcode-api.apiwise.nl/v2/addresses/?postcode=1118CP&number=202"}}}')]);

        $address = $this->postcodeApiNu->findByPostcodeAndHouseNumber('1118CP', '202');
    ...
    {
        Http::fake(['https://postcode-api.apiwise.nl/v2/addresses/?postcode=1234AB&number=123' => Http::response('{"_embedded":{"addresses":[]},"_links":{"self":{"href":"https://postcode-api.apiwise.nl/v2/addresses/?postcode=1234AB"}}}')]);

        $address = $this->postcodeApiNu->findByPostcodeAndHouseNumber('1234AB', '123');
