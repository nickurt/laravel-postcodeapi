...
class TomTomTest extends BaseProviderTest
{
    ...
    {
        $this->tomTom = (new TomTom($this->httpClient = new \nickurt\PostcodeApi\Http\Guzzle6HttpClient()))
    ...
    public function it_can_get_the_correct_values_for_find_an_invalid_postal_code()
    {
        $this->httpClient->setHttpClient(new Client([
            'handler' => new MockHandler([
                new Response(200, [], '{"summary":{"query":"zeroresults","queryType":"NON_NEAR","queryTime":36,"numResults":0,"offset":0,"totalResults":0,"fuzzyLevel":2},"results":[]}')
            ]),
        ]));
